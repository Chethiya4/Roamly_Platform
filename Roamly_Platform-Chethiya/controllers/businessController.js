...
const registerBusiness = asyncHandler(async (req, res, next) => {
    ...
    try {
        ...
        // Parse facilities — accept native Array or single string
        let facilities = [];
        ...
        if (latitude && isNaN(parsedLat)) {
            return res.status(400).json({ success: false, message: 'Google Maps Latitude must be a valid decimal number (e.g. 6.9271). Do not use degrees/minutes/seconds.' });
        }
        if (longitude && isNaN(parsedLng)) {
            return res.status(400).json({ success: false, message: 'Google Maps Longitude must be a valid decimal number (e.g. 79.8612). Do not use degrees/minutes/seconds.' });
        }
        ...
        try {
            await Notification.create({
                ...
                message: `New business registered: ${business.name}`,
        ...
        if (req.user && req.user.email) {
            try {
                await sendEmail(
                    ...
                    `<h2>Registration Received</h2><p>We've received your registration for ${business.name}. Our team will review it and you'll hear back soon.</p>`
                );
            } catch (err) {
                console.error('Failed to send business registration email:', err.message);
            }
        }
