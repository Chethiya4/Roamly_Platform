    ...
    if (business.owner && business.owner.email) {
        try {
            await sendEmail(
                ...
                `<h2>Business Approved</h2><p>Good news — ${business.name} has been approved and is now live on Roamly!</p>`
            );
        } catch (err) {
...
const rejectBusiness = asyncHandler(async (req, res) => {
    ...
    if (business.owner && business.owner.email) {
        try {
            await sendEmail(
                business.owner.email,
                'Update on your Roamly Business Registration',
                `<h2>Action Required</h2><p>Your registration for ${business.name} needs some changes: ${reason}. You can update your details and resubmit from your business dashboard.</p>`
            );
        } catch (err) {
...
const approveSpot = asyncHandler(async (req, res) => {
    ...
    await spot.populate('submittedBy', 'name email');
    if (spot.submittedBy && spot.submittedBy.email) {
        try {
            await sendEmail(
                spot.submittedBy.email,
                'Tourist Spot Approved',
                `<h2>Spot Approved</h2><p>Your tourist spot submission, ${spot.name}, has been approved and is now visible on Roamly!</p>`
            );
        } catch (err) {
            console.error('Failed to send spot approval email:', err.message);
        }
    }
...
const rejectSpot = asyncHandler(async (req, res) => {
    ...
    if (spot.submittedBy && spot.submittedBy.email) {
        try {
            await sendEmail(
                spot.submittedBy.email,
                'Tourist Spot Update Required',
                `<h2>Action Required</h2><p>Your tourist spot submission, ${spot.name}, needs some changes: ${reason}. You can edit and resubmit it from your account page.</p>`
            );
        } catch (err) {
            console.error('Failed to send spot rejection email:', err.message);
        }
    }
